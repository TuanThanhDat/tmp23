...
cur_dir = os.path.join(os.getcwd(),'Image-Adaptive-YOLO')
...
parser.add_argument('--exp_dir', dest='exp_dir', default=f'{cur_dir}/experiments', help='models are saved here')
parser.add_argument('--gpu_id', dest='gpu_id', type=str, default='1', help='if use gpu, use gpu device id')
...
parser.add_argument('--vocfog_traindata_dir', dest='vocfog_traindata_dir', default=f'{cur_dir}/data/vdd/liuwenyu/data_vocfog/train/JPEGImages/',
                    help='the dir contains ten levels synthetic foggy images')
parser.add_argument('--vocfog_valdata_dir', dest='vocfog_valdata_dir', default=f'{cur_dir}/data/vdd/liuwenyu/data_vocfog/val/JPEGImages/',
                    help='the dir contains ten levels synthetic foggy images')
parser.add_argument('--train_path', dest='train_path', nargs='*', default=f'{cur_dir}/data/dataset_fog/voc_norm_train.txt', help='folder of the training data')
parser.add_argument('--val_path', dest='val_path', nargs='*', default=f'{cur_dir}/data/dataset_fog/voc_norm_test.txt', help='folder of the training data')
parser.add_argument('--test_path', dest='test_path', nargs='*', default=f'{cur_dir}/data/dataset_fog/quick_test.txt', help='folder of the training data')
parser.add_argument('--class_name', dest='class_name', nargs='*', default=f'{cur_dir}/data/classes/vocfog.names', help='folder of the training data')
parser.add_argument('--WRITE_IMAGE_PATH', dest='WRITE_IMAGE_PATH', nargs='*', default=f'{cur_dir}/experiments/exp_101/detection_results/', help='folder of the training data')
parser.add_argument('--WEIGHT_FILE', dest='WEIGHT_FILE', nargs='*', default=f'{cur_dir}/experiments/exp_101/checkpoint/yolov3_test_loss=5.8980.ckpt-75', help='folder of the training data')
parser.add_argument('--pre_train', dest='pre_train', default='NULL', help='the path of pretrained models if is not null. not used for now')
                                                                            #./experiments/exp_101/checkpoint/yolov3_test_loss=5.8980.ckpt-75.data-00000-of-00001
...
__C.YOLO.ANCHORS                = f"{cur_dir}/data/anchors/coco_anchors.txt"
